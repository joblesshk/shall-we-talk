...
final class TodoStore: ObservableObject {
    ...
    private func restoreCalendarEvent(for item: TodoItem) {
        ...
        Task { @MainActor [weak self] in
            guard let identifier = await TodoCalendarScheduler.shared.upsert(todo: item, plan: plan)
            else { return }
            self?.setCalendarEventIdentifier(identifier, for: item.id)
        }
    }
    ...
    func replaceRecordingGroup(containing item: TodoItem, with texts: [String]) -> TodoGroupReplacement? {
        let cleaned = texts
            .map { $0.trimmingCharacters(in: .whitespacesAndNewlines) }
            .filter { !$0.isEmpty }
...
final class TodoCalendarScheduler {
    ...
    func upsert(todo: TodoItem, plan: TodoCalendarPlan?) async -> String? {
        ...
        do {
            ...
            DiagLog.log("calendar", "写入失败: \(error.localizedDescription)")
            return nil
        }
    }
    ...
    private func ensureAccess() async -> Bool {
        ...
        case .notDetermined:
            do { return try await eventStore.requestFullAccessToEvents() }
                ...
                DiagLog.log("calendar", "权限请求失败: \(error.localizedDescription)")
                return false
            }
        case .writeOnly:
