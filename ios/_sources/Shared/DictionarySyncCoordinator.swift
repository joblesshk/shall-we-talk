...
enum DictionarySyncCoordinator {
    ...
    static func syncAndMerge(currentManual: [String], currentAuto: [String], currentBlocked: Set<String>,
        ...
        // 只把本轮新进入屏蔽名单的词/纠错对补成墓碑。否则 90 天后被正常清理的墓碑会因
        // 本机永久 blocklist 仍含该项而在下一轮重新生成，保留期将永远无法生效。
    ...
    /// 供 ASR 热词 context 与整理 prompt(含修改模式词典块,不含纠错对块——见
    /// `PromptBuilder.buildEditPass` 的注释)消费:手动添加 ∪ 本机实时挖掘 ∪ 云端已合并的
    /// 纠错对,按 source 去重(手动优先于实时挖掘,反映用户明确意图;实时挖掘优先于云端
    /// 旧快照,因为它反映最新的本地历史),截断到 limit。只读本机 UserDefaults 缓存,
    /// 不触碰 iCloud,可在 MainActor 上同步调用。
    ///
    /// `blocked`(2026-08-19 新增):不分来源统一过滤掉用户删除过的 source——本机实时挖掘
    /// 那一路转发给 `DictionaryMiner.correctionPairs` 防止重新挖出,这里再统一过滤一遍
    /// 兜底云端快照里还没跑完一轮同步、墓碑尚未落地的情况,保证"删了立刻不再生效"。
