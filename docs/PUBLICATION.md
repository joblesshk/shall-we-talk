...
The initial publication retained the original proprietary license. The owner subsequently authorized open-source licensing: the current revision uses GPL-3.0-only for original project materials, with third-party terms preserved. See LICENSING.md. Source licensing does not by itself settle every binary distribution obligation.
...
## Open-source transition (2026-09-14)

The owner authorized selection and application of an open-source license. The original code, scripts, documentation and original assets are now GPL-3.0-only, unless separately identified. LICENSE contains the unmodified GNU text; COPYRIGHT declares the project grant and scope. The original MIT, Apache, GPL and other third-party notices and hashes are preserved. The change is prospective from this source revision and does not retroactively relabel older binary releases. No application behavior, credentials, signing or deployment changed.
